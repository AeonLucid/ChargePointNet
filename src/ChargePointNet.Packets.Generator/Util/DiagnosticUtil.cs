...
internal static class DiagnosticUtil
{
    ...
    private static readonly DiagnosticDescriptor DescriptorInvalidObject = new(
        "PKT3", 
        "title", 
        $"An object was referenced that does not implement {Constants.PacketNamespace}.{Constants.PacketBinaryType} or {Constants.PacketNamespace}.{Constants.PacketHexType}", 
        "PacketGenerator", 
        DiagnosticSeverity.Error, 
        true);
