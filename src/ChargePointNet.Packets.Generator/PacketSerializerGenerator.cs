...
using System.Linq;
using System.Text;
using ChargePointNet.Packets.Generator.CodeGen;
using ChargePointNet.Packets.Generator.Extensions;
using ChargePointNet.Packets.Generator.Util;
...
// #endif
    ...
    private static string BuildFile(BuildContext build)
    {
        ...
        var isHex = build.Syntax.BaseList?.Types.Any(x => x.Type.ToString() == Constants.PacketHexType) ?? false;
        
        // var binaryFormat = build.SemanticModel.get
        ...
        // Size
        builder.AppendFormat("{0}public int Size()\n", ident);
        builder.AppendFormat("{0}{{\n", ident);
        BuildMethod(build, builder, ident + Constants.DefaultIdent, new CodeGenSize(isHex));
        builder.AppendFormat("{0}}}\n", ident); // Size
        builder.AppendFormat("\n");

        // Serialize
        builder.AppendFormat("{0}public void Serialize(ref global::ChargePointNet.Packets.HexSpanWriter writer)\n", ident);
        builder.AppendFormat("{0}{{\n", ident);
        ...
        // Parse
        builder.AppendFormat("{0}public bool Deserialize(ref global::ChargePointNet.Packets.HexSpanReader reader)\n", ident);
        builder.AppendFormat("{0}{{\n", ident);
    ...
    private static void BuildMethod(BuildContext build, StringBuilder builder, string ident, ICodeGen codeGen)
    {
        ...
        {
            builder.AppendFormat("{0}int {1} = 0;\n", ident, CodeGenSerialize.BytesSizeRef);
        }
        else if (isParse)
        {
    ...
    private static void WriteProperty(
        ...
        TypeSyntax propType,
        string? propName,
        PacketFieldAttribute propAttribute)
    {
        ...
        else if (propKind == SyntaxKind.ArrayType)
        {
            ...
            var elTypeName = typeArrElement.GetTypeName();
            var elKind = typeArrElement.Kind();
            if (elKind == SyntaxKind.PredefinedType && elTypeName == "byte" && propAttribute.Raw)
            {
                codeGen.WriteBytes(builder, ident, propName, propAttribute);
            }
            else
            {
